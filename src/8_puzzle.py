"""
A program to solve any 8-puzzle problem using the A* algorithm. The user can
enter the start state and goal state of their 8-puzzle problem, and select
either the Manhattan Distance heuristic or the Hamming Distance heuristic to
use with the A* algorithm.
"""
...
def choose_states():
    """
    Takes user input for the start state and desired goal state of the board.
    ...
    while True:
        start_input = input(
            "\nPlease enter the start state in a single line containing nine "
            "unique integers 0-8, from top left to bottom right. For example, "
            "'012345678', where '0' represents the blank tile:\n")
        if sorted(start_input) == sorted("012345678"):
            break
        else:
            print("This is not a valid board state. Please try again!")
    while True:
        goal_input = input(
            "\nPlease enter the goal state in the same format:\n")
        if (sorted(goal_input) == sorted("012345678") and
                goal_input != start_input):
            break
        elif goal_input == start_input:
            print("The goal state cannot be the same as the start state. "
                  "Please try again!")
        else:
            print("This is not a valid board state. Please try again!")

    start = np.array(list(map(int, start_input)))
    goal = np.array(list(map(int, goal_input)))
